...
def get_malignant_probability(model, X):
    ...
    try:
        ...
        # Try various ways to detect malignant class index
        mal_idx = None
        for i, c in enumerate(classes):
            ...
            if s.startswith("m") or s.startswith("mal") or s == "1" or s == "1.0":
                mal_idx = i
                break
        # fallback: assume second column is malignant for binary classifiers
...
def chatbot_medical(query):
    ...
    # =====================================================================
    # 2. Jika dataframe ada, pastikan kolom Prediction ada; jika belum, coba prediksi
    # =====================================================================
    try:
    ...
    if "pola" in query or "insight" in query or "fitur berpengaruh" in query:
        # berikan contoh fitur yang biasanya berpengaruh
...
if mode == "Mode Tenaga Medis":
    ...
    if file is not None:
        ...
        if df is not None:
            ...
            # Download hasil prediksi (pastikan string tidak terpotong -> gunakan label=)
            try:
...
# ======================================
    ...
    **Catatan Penting:**  
    ...
    if st.button("🔍 Dapatkan Indikasi Sekarang"):
        ...
        # -----------------------------
        # Buat dummy fitur default (0.0)
        dummy = {feat: 0.0 for feat in feature_list}

        # Heuristik sederhana (bukan klinis, hanya untuk screening awal)
        # radius_mean: tergantung ukuran (skalakan)
        radius = float(size_cm)
        dummy["radius_mean"] = 10.0 + radius * 2.5  # baseline + skala
        ...
        # concavity/concave points increase if nipple discharge or skin change present
        ...
        # Ensure all required features exist in dummy (if model needs specific names, keep zeros if absent)
        ...
        try:
            ...
            prob_m = get_malignant_probability(model, df_input)  # may be None

            # Normalize pred label textual
            ...
            if pred_label == "1" or pred_label.lower().startswith("m") or pred_label.lower().startswith("mal"):
                ...
                # fallback: if prob available use threshold
